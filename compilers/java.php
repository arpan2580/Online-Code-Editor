...
<?php
	...
	$code=$_POST["code"];
	$input=$_POST["input"];
	$filename_code="code.java";
	$filename_in="input.txt";
	$filename_error="error.txt";
	$runtime_file="runtime.txt";
	$executable="dir * /b | findstr \.class$";
  ...
  $check=0;
	...
	$executable=shell_exec($executable);
	$out=$out." ".strstr($executable,'.class',true);
	...
	if(trim($error)=="")
	{
		...
		{
			$out=$out." ".file_get_contents($filename_in);
			$output=shell_exec($out);
		}
		?>
        <div style="width: 60%">
