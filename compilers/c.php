...
<?php
	...
	$code=$_POST["code"];
	$input=$_POST["input"];
	$filename_code="code.c";
	$filename_in="input.txt";
	$filename_error="error.txt";
	$executable="code.exe";
	$command=$CC." ".$filename_code." -o code";	
	...
	if(trim($error)=="")
	{
		...
		{
			$executable=$executable." ".file_get_contents($filename_in);
			$output=shell_exec($executable);
		}
		?>
        <div style="width: 60%">
	...
	else if(!strpos($error,"error"))
	{
        ...
        <div style="width: 60%">
          <textarea class="lined" name="code" readonly><?php echo $error ?></textarea>
					<!--<a href="ide.html"><button class="button">Go Back</button></a>-->
        </div>
	<?php
		//echo "<pre>$error</pre>";
		if(trim($input)=="")
	...
	if($check==1)
	{
		echo "<pre>Verdict : Code Error</pre>";
	}
	...
	{
		echo "<pre>Verdict : Ran Successfully</pre>";
	}
